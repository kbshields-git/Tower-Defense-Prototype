    ...
    public class RTS_CameraEditor : Editor
    {
        ...
        private void HeightTab()
        {
            ...
            if (camera.useScrollwheelZooming || camera.useKeyboardZooming)
            {
                ...
                using (new HorizontalBlock())
                {
                    camera.angleAtMinY = EditorGUILayout.Vector3Field("Angle Min Height: ", camera.angleAtMinY);
                }
                using (new HorizontalBlock())
                {
                    camera.angleAtMidY = EditorGUILayout.Vector3Field("Angle Mid Height: ", camera.angleAtMidY);
                }
                using (new HorizontalBlock())
                {
                    camera.angleAtMaxY = EditorGUILayout.Vector3Field("Angle Max Height: ", camera.angleAtMaxY);
                }
            }  
        }
    }
