...
namespace RTS_Cam
{
    ...
    public class RTS_Camera : MonoBehaviour
    {
        ...
        public float minHeight = 7f; //minimnal height
        ...
        private void Rotation()
        {
            ...
            float midY = Mathf.Lerp(minHeight, maxHeight, 0.7f);
            if (m_Transform.position.y > midY)
            {
                m_Transform.rotation = Quaternion.Lerp(Quaternion.Euler(angleAtMidY), Quaternion.Euler(angleAtMaxY), m_Transform.position.y / maxHeight);
            }
            else if (m_Transform.position.y <= midY)
            {
                m_Transform.rotation = Quaternion.Lerp(Quaternion.Euler(angleAtMinY), Quaternion.Euler(angleAtMidY), m_Transform.position.y / midY);
            }
        }
